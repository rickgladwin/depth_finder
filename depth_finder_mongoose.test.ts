...
describe('getDepth', () => {
    ...
    it('returns 1 for an object containing an ObjectId value', () => {
        const objectIdString: string = '613ba850bb8687293ebf0678';
        const exampleObjectId = Types.ObjectId(objectIdString); //must be string of 12 bytes or 24 hex chars
        const objectWithObjectIdValue = {
            key1: 'value1',
            key2: exampleObjectId,
        }
        expect(getDepth(objectWithObjectIdValue)).toEqual(1);
    });
});
describe('isKeyValueObject', () => {
    ...
    it('returns false for an ObjectId', () => {
        // mongoose.Types.ObjectId displays its 'id' property as a string when printed,
        // due to duck typing, BUT typeof ObjectId === 'object'.
        const objectIdString: string = '613ba850bb8687293ebf0678';
        const stringPrintingObject = Types.ObjectId(objectIdString); //must be string of 12 bytes or 24 hex chars

        expect(typeof stringPrintingObject).toEqual('object');
        expect(JSON.parse(JSON.stringify(stringPrintingObject))).toEqual(objectIdString);
        expect(stringPrintingObject instanceof Types.ObjectId).toEqual(true);
        expect(isKeyValueObject(stringPrintingObject)).toEqual(false);
    });
    it('returns true for an object containing an ObjectId as a value', () => {
        const objectIdString: string = '613ba850bb8687293ebf0678';
        const exampleObjectId = Types.ObjectId(objectIdString); //must be string of 12 bytes or 24 hex chars
        const objectWithObjectIdValue = {
            key1: 'value1',
        ...
        expect(isKeyValueObject(objectWithObjectIdValue)).toEqual(true);
    })
});
...
describe('compare', () => {
    ...
    const objectId1 = Types.ObjectId();
    const objectId2 = Types.ObjectId();
    ...
    const comparisonExamples = [
        ...
        {
            dataTypeName: 'mongoose.Types.ObjectId',
            dataTypeExample1: objectId1,
            dataTypeExample2: objectId2,
        },
        {
